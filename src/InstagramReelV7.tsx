...
const WordByWordSubtitle: React.FC<{
  ...
  const totalDuration = words.length * framesPerWord + 30; // Extra frames at end
    ...
    <div style={{
      ...
      <div style={{
        ...
        {words.map((word, i) => {
          ...
          // Highlight effect - word glows briefly when it appears
...
const HookScreen: React.FC = () => {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, backgroundColor: theme.bg, fontFamily }}>
      ...
      <div style={{ color: theme.text, fontSize: 20, fontWeight: 600, marginTop: 16, opacity: interpolate(frame - 60, [0, 15], [0, 1], { extrapolateRight: 'clamp' }), fontFamily }}>SmallERP</div>
    </div>
  );
};
...
const MiniChart: React.FC<{ delay: number }> = ({ delay }) => {
  ...
  return (
    <svg width={w} height={h}>
      <defs>
        <linearGradient id="mg7" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={theme.primary} stopOpacity="0.3" /><stop offset="100%" stopColor={theme.primary} stopOpacity="0" /></linearGradient>
        <clipPath id="mc7"><rect x="0" y="0" width={w * progress} height={h} /></clipPath>
      </defs>
      <g clipPath="url(#mc7)"><path d={`${path} L ${w} ${h} L 0 ${h} Z`} fill="url(#mg7)" /><path d={path} fill="none" stroke={theme.primary} strokeWidth="2" /></g>
    </svg>
  );
};
...
// Subtitles with word-by-word timing
const subtitleData = [
  { words: ["Running", "a", "business", "in", "the", "UAE?"], start: 15, fps: 5 },
  { words: ["Here's", "your", "all-in-one", "solution"], start: 96, fps: 6 },
  { words: ["SmallERP", "gives", "you", "real-time", "visibility"], start: 171, fps: 5 },
  { words: ["Revenue,", "invoices,", "customers,", "inventory"], start: 261, fps: 6 },
  { words: ["One", "dashboard"], start: 366, fps: 8 },
  { words: ["Create", "VAT-compliant", "invoices", "in", "seconds"], start: 426, fps: 5 },
  { words: ["And", "with", "AI", "Copilot,", "just", "ask:"], start: 531, fps: 4 },
  { words: ["Show", "me", "unpaid", "invoices"], start: 606, fps: 5 },
  { words: ["Instant", "answers,", "one-click", "actions"], start: 681, fps: 5 },
  { words: ["Start", "free", "at", "smallerp.ae"], start: 771, fps: 5 },
];

export const InstagramReelV7: React.FC = () => {
  return (
    <AbsoluteFill style={{ fontFamily }}>
      <Sequence from={0} durationInFrames={120}><IPhoneFrame><HookScreen /></IPhoneFrame></Sequence>
      <Sequence from={120} durationInFrames={180}><IPhoneFrame><DashboardScreen /></IPhoneFrame></Sequence>
      <Sequence from={300} durationInFrames={180}><IPhoneFrame><InvoiceScreen /></IPhoneFrame></Sequence>
      <Sequence from={480} durationInFrames={240}><IPhoneFrame><CopilotScreen /></IPhoneFrame></Sequence>
      <Sequence from={720} durationInFrames={180}><IPhoneFrame><CTAScreen /></IPhoneFrame></Sequence>
